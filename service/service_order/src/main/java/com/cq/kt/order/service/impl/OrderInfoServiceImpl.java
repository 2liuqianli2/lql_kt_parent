package com.cq.kt.order.service.impl;


import com.cq.kt.model.order.OrderDetail;
import com.cq.kt.model.order.OrderInfo;
import com.cq.kt.order.mapper.OrderInfoMapper;
import com.cq.kt.order.service.OrderDetailService;
...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
    ...
    public Map<String, Object> findPageOrderInfo(Page<OrderInfo> pageParam,
        ...
        if(!StringUtils.isEmpty(outTradeNo)) {
            wrapper.eq("out_trade_no",outTradeNo.trim());
        }
        if(!StringUtils.isEmpty(phone)) {
            wrapper.eq("phone",phone.trim());
        }
        if(!StringUtils.isEmpty(createTimeBegin)) {
            wrapper.ge("create_time",createTimeBegin.trim());
        }
        if(!StringUtils.isEmpty(createTimeEnd)) {
            wrapper.le("create_time",createTimeEnd.trim());
        }
        //调用实现条件分页查询
